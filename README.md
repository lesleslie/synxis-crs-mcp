...
- [Installation via Claude Code marketplace](#installation-via-claude-code-marketplace)

## Installation via Claude Code marketplace

This repo is packaged as a Claude Code plugin. Register the [www-mcp-servers marketplace](https://github.com/lesleslie/www-mcp-servers) with Claude Code, then install the `synxis-crs` plugin to expose the slash commands (`/synxis-crs-lookup`, `/synxis-crs-availability`, `/synxis-crs-reservation`) and connect to the running MCP server at `http://localhost:3046/mcp`. The plugin self-references its colocated `.mcp.json`, so no additional client configuration is required.
